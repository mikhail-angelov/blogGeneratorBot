...
const INIT_DATA = [
  {
    id: "1",
    subject: "Cool news",
    body: "This blog is generated by Telegram Bot @blogGeneratorBot",
  },
];

function createSite(user, repo) {
  return github
    .createRepo(user, repo)
    .then(() => (user.repo = repo))
...
function removeSite(user) {
  return github.removeRepo(user, user.repo).then(() => {
    user.repo = null;
  });
}
...
function copyToRepo(user, fileName, data) {
  const content =
    data || fs.readFileSync(BASE + fileName, { encoding: "utf8" });
  return github.createFile(user, fileName, content);
}

function updateSite(user, data) {
  return github
    .getFile(user, conf.templateFileName)
